...
class ahwi
{
     ...
     * @var string
     */
    protected string $sAbout = "AXELS PHP WEB INSTALLER";
    ...
    /**
     * Array of checks for requirements
     * @var array
     */
    protected array $aChecks = [];

    /**
     * Array of VCS tools and commands for some actions
     * @var array
     */
    protected $aVcs = [
        'git' => [
     ...
     * @return string
     */
    private function _httpGet(string $url, bool $bHeaderOnly = false)
    {
        $ch = curl_init($url);
        if ($bHeaderOnly) {
     ...
     * @return array
     */
    private function _setConfig(array $aCfg = []): array
    {
        // verify array
        $sErrors = '';
     ...
     * @return string
     */
    private function _getZipfilename(): string
    {
     ...
     */
    protected function _moveIfSingleSubdir($sSubdir, $aEntries): void
    {
     ...
     * @return string
     */
    public function getTmpzip(): string
    {
    ...
    /**
     * Check if the installer was started as root
     * @return boolean
     */
    private function _checkDenyroot(): bool
    {
    ...
    /**
     * Check if all php modules were found
     * @return boolean
     */
    function _checkModules(): bool
    {
        if (
            isset($this->aCfg['checks']['phpextensions'])
    ...
    /**
     * Check php version ; if version is too low then $this->aErrors gets a new item
     * @return void
     */
    function _checkPhpversion(): void
    {
        if (
            isset($this->aCfg['checks']['phpversion'])
    ...
    */
     ...
     * @return bool
     */
    function download(bool $bForce = true): bool
    {
     ...
     */
    function vcsDetect($sType = false): mixed
    {
        $aReturn = [];
     ...
     * @param  string  $sType subdir of vcs, ie. ".git" for git
     * @return array|bool
     */
    function vcsUpdate(string $sType): array|bool
    {
     ...
     * @return boolean
     */
    function verifyDownload(string $md5OfData, string $sUrl = ''): bool
    {
        $sMd5Url = ($sUrl
            ? $sUrl
